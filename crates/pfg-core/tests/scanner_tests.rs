...
fn test_scan_file_pipeline_with_masking() {
    ...
    let options_masked = ScanOptions { include_values: false };
    ...
    assert_eq!(report_masked.findings[0].display_value, Some("Se***".to_string()));

    // 2. Scan with include_values = true -> display_value should NOT be masked
    let options_unmasked = ScanOptions { include_values: true };
    let report_unmasked = scan_file(&test_file, &options_unmasked).expect("Scan should succeed");

    assert_eq!(report_unmasked.findings[0].display_value, Some("Secret author".to_string()));
